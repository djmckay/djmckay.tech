...
export function updateInput(table, r, nowIso) {
  return {
    ...
    UpdateExpression:
      "ADD games :one, wins :w, losses :l, stopped :s, cells :cells, calls :calls, secs :secs, costMicro :cost, checks :checks, flagged :fl, approvedWrong :aw, rejectedFine :rf " +
    ...
    ExpressionAttributeValues: {
      ...
      ":cells": N(r.cells), ":calls": N(r.calls), ":secs": N(r.secs), ":cost": N(r.costMicro), ":checks": N(r.checks),
      ":fl": N(r.flagged), ":aw": N(r.approvedWrong), ":rf": N(r.rejectedFine),
